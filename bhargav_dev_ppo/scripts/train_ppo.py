...
Network input (124 = 4 + k_triples*12 at defaults):
  env vector (4):  [x/8, y/8, cos(yaw), sin(yaw)]  -- no object positions
  kg vector (120): up to k_triples=10 perceived triples; per triple
...
(existing runs and checkpoints are never overwritten):
    ...
    Optional:
        kg=latest iterations=150 horizon=2048 k_triples=10 perceive_every=75
...
def train():
    ...
    for it in range(1, iterations + 1):
        ...
        print(
            ...
            f"water={stats['water_rate']:.2f} poison={stats['poison_rate']:.2f} "
