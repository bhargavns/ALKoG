...
def save_training_curves(rows, plot_dir):
    ...
    if "water_rate" in rows[0]:
        axes[1, 0].plot(it, [r["water_rate"] for r in rows], color=SERIES[2], label="water")
    if "poison_rate" in rows[0]:
        axes[1, 0].plot(it, [r["poison_rate"] for r in rows], color=SERIES[4], label="poison")
...
def save_trajectory_plot(trajectories, plot_dir, filename="trajectories.png", arena_half=7.0):
    ...
    for ax, traj in zip(axes.flat, trajectories):
        ...
        if "water" in traj:
            ax.scatter(*traj["water"], marker="D", s=60, color=SERIES[2], zorder=4)
        if "poison" in traj:
            ax.scatter(*traj["poison"], marker="P", s=70, color=SERIES[4], zorder=4)
