...
class BoxProposer:
    ...
    def propose_full(self, frame):
        """Return boxes, masks, and SAM quality metadata for diagnostics."""
        h, w = frame.shape[:2]
        img_area = float(h * w)
...
class PerceptionPipeline:
    ...
    def detect_full(self, frame):
        """Return boxes, boolean masks, embeddings, and SAM quality metadata.

        The original ``detect`` API is retained for the legacy KG pipeline.
        Dataset generation and the soft-category path use this richer API.
        """
