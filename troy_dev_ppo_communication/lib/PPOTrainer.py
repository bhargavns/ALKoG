...
class PPOTrainer:
    ...
    def _reset_env(self):
        ...
        if self.record_trajectories:
            ...
            if getattr(self.env, "resources", False):
                for name in ("water", "poison"):
                    self._traj[name] = self.env.model.body(name).pos[:2].copy().tolist()
    ...
    def _finish_trajectory(self, info, truncated):
        ...
        if info.get("food_reached", False):
            ...
            outcome = "death"
        elif info.get("water_reached", False):
            outcome = "water"
        elif info.get("poison_touched", False):
            outcome = "poison"
        else:
    ...
    def collect_and_update(self):
        ...
        # only meaningful when the env runs with resources=True; stay 0 otherwise
        waters, poisons = 0, 0
        ...
        for t in range(self.horizon):
            ...
            if terminated or truncated:
                ...
                waters += int(info.get("water_reached", False))
                poisons += int(info.get("poison_touched", False))
        ...
        return {
            ...
            "water_rate": waters / n_eps,
            "poison_rate": poisons / n_eps,
