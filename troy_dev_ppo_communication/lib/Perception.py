...
class BoxProposer:
    ...
    def __init__(
        ...
        if not os.path.isfile(checkpoint):
            raise FileNotFoundError(
                f"SAM checkpoint not found: {checkpoint}. Run "
                "`python scripts/download_sam_checkpoint.py` first."
            )
    ...
    def propose(self, frame):
        ...
        boxes, seg_masks, _quality = self.propose_full(frame)
        return boxes, seg_masks

    def propose_full(self, frame):
        """As `propose`, plus SAM's own quality metadata per proposal.

        Returns (boxes, masks, quality) where quality[i] carries SAM's
        predicted_iou, stability_score, and pixel area for proposal i. The
        soft-category dataset generator uses these to filter and diagnose
        proposals; the KG path ignores them and calls `propose`.
        """
        h, w = frame.shape[:2]
        img_area = float(h * w)
        boxes, seg_masks, quality = [], [], []
        ...
        return boxes, seg_masks, quality
...
class PerceptionPipeline:
    ...
    def detect_full(self, frame):
        """Returns (boxes, masks, embeddings, quality) -- the same embedding as
        `detect`, plus SAM's per-proposal quality metadata.

        Note the argument order differs from `detect` (masks before embeddings):
        this is the soft-category path's API, kept as-is so the category dataset
        and grounding code read the same way here as in the reference tree.
        """
        boxes, masks, quality = self.proposer.propose_full(frame)
        cnn = self.encoder.encode(frame, boxes)
        if not boxes:
            empty = torch.empty(0, EMBEDDING_DIM, device=self.device)
            return boxes, masks, empty, quality
        hists = torch.stack(
            [
                torch.from_numpy(mask_color_histogram(frame, mask)).to(self.device)
                for mask in masks
            ]
        )
        weight = float(np.sqrt(0.5))
        embeddings = torch.cat([weight * cnn, weight * hists], dim=1)
        return boxes, masks, embeddings, quality
