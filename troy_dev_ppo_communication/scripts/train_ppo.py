...
from lib.Oracle import geom_body_table
from lib.OracleGrounding import (
    PERCEPTION_MODES,
    build_oracle_kg,
    oracle_bodies,
    perceive_scene_oracle,
...
required_arguments = []
optional_arguments = {
    "kg": "latest",           # path to phase-1 kg.pt, or 'latest' for newest run. Not used in most recent version. 
    "iterations": "150",
    "horizon": "2048",
    "k_triples": "6",
    ...
    "model": "slot",          # only architecture in tree; transformer retired to Archive/
    ...
    "perception": "sam",      # sam | softcat | oracle_id | oracle_full
    "category_model": os.path.join(_OUTPUT, "category_model.pt"),  # perception=softcat
    "resources": "0",         # 1 = 5-object world (adds water/poison)
    "videos": "10",           # agent's-eye recordings, evenly spaced over training
}
...
(existing runs and checkpoints are never overwritten):
    ...
    videos/                          agent's-eye mp4 (4-cam panorama, live
                                     ...
                                     policy rollout at `videos` evenly spaced
    ...
    Optional:
        ...
        lr=3e-4 checkpoint_every=25 out_dir=.../output/runs run_name= videos=10
...
def train():
    ...
    perception = g_ArgParse.get("perception")
    ...
    if model_kind != "slot":
        raise ValueError(
            f"model must be 'slot', got {model_kind!r}. The transformer policy was retired\n"
            "to Archive/lib/TransformerPolicy.py -- it does not learn on real perception\n"
            "(RESULTS.md sections 6 and 8). Run it from there if you need to reproduce it."
        )

    if perception == "sam" and kg_path == "latest":
    ...
    # agent's-eye videos at n_videos evenly spaced points: 0 (untrained) ... final
    record_iters = set(record_points(iterations, n_points=n_videos))
    ...
    category_model = None
    # kg_trained.pt is only written when a run finishes; save the KG up front too
    # so mid-run checkpoints from an interrupted long run stay loadable without
    # having to reproduce the synthetic KG from the seed.
    kg_init_path = os.path.join(run_dir, "kg_init.pt")
    if perception == "sam":
        ...
        table = geom_body_table(env.model) if perception in ("oracle_id", "oracle_full") else None
        kg_path = f"<{perception}>"
        print(f"KG: one node per body {bodies} -> ids {body_to_node}")
        if perception == "softcat":
    ...
    kg.save(kg_init_path)
    print(f"Saved initial KG for checkpoint reload: {kg_init_path}")

    # oracle_full never proposes boxes, so SAM is only loaded when something uses it
    if perception == "oracle_full":
        pipeline = None
        print("Skipping SAM load (oracle_full takes boxes from the segmentation image)")
    else:
    ...
    if perception == "sam":
        ...
        def per_frame_fn():
            if perception == "softcat":
                return perceive_scene_softcat(
                    env, pipeline, kg, category_model, body_to_node
                )
            return perceive_scene_oracle(
                env, pipeline, kg, table, body_to_node, mode=perception
            )

        def triple_fn():
            return assemble_triples_geo(kg, per_frame_fn(), k=k_triples)

    ModelCls = TripleActorCritic
    ...
    def record_video(it):
        ...
        outcome, ep_ret = record_policy_episode(
            ...
            tag=f"iter {it}",
            per_frame_fn=None if perception == "sam" else per_frame_fn,
        )
